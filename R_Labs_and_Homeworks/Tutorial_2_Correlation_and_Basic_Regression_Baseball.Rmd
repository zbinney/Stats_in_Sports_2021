...
The only negative of the locally estimated scatterplot smoothing (LOESS) model is if we were to look at the coefficients for the line of best fit it would be more complicated - which is why we aren't doing that today. It's outside the scope of this course. But... 
...
```{r corr^2, include = FALSE}
# Plot a line of best fit through the data
teams_21c <- teams_21c %>% 
  mutate(rundiff2 = (R - RA)^2)

cor(teams_21c$rundiff2, teams_21c$W)
...
When we investigated the correlation of multiple variables with wins above we were using them what I would call *descriptively*. For example, can we use runs scored to **describe** wins scored in the same year?
...
Here are the questions for your HW 2. You should answer these in an RMarkdown document, knit to **PDF** and submit on Canvas.
...
* Use **visual evidence** and **correlation coefficients** to figure out which is most strongly (linearly) associated with runs scored at the team level. HINT: construct a correlation matrix. (2 pts)

* Write a couple sentences explaining which statistic you would use to prioritize hitters and why based on the results above (1 pt)
...
* To get at that calculate the year-over-year repeatability of both your statistic *and BA* within the same player using the `Batting` data frame in Lahman. Consider only batting seasons with >50 ABs and use the same time period as above (1982-2002, except 1994). Below is some "shell code" to help you create the data frame you need to answer this question. (2 pts)

```{r hwq3_dataprep, eval = FALSE}

# Calculate and add subsequent year's batting statistics
batting_ytoy <- Batting %>% 
  
  ### Code for filtering data frame by year and ABs, and creating new variables for your statistics ###
  ### (which you should be able to copy from above) goes here.                                      ###


  # Sorting the data so we can add new columns for next years' stats
  arrange(playerID, yearID) %>% 
  group_by(playerID) %>% 
  ...
  filter(!is.na(BA_next)) # Drop all seasons without next BA (that is, each player's last season during this time)

```

* Which statistic (BA or your choice) has greater repeatability? (0.5 pts)

* Does "higher" or "lower" year-over-year repeatability suggest a statistic is a better reflection of individual contributions, and why? Explain in about 2 sentences. (0.5 pts)
