...
players18 <- shots18 %>% 
  ...
  summarize(FGA = n(), # Count rows to get FGA
            ...
            pct_3pt = FGA_3pt/FGA,
            dist_2pt = mean(SHOT_DISTANCE[pts == 2])) %>%
...
players18 %>% 
  ...
  geom_text(aes(label = PLAYER_NAME)) +
  labs(title = "Mr. Analytics, 2018-19 NBA Season",
...
# Create one scatterplot
base_court + 
  geom_point(data = shots18 %>% filter(PLAYER_NAME == "Domantas Sabonis"), # Do filtering inside geom_point()
...
shots18 %>%
  group_by(SHOT_TYPE) %>% 
  summarize(FG_pct = mean(SHOT_MADE_FLAG))
